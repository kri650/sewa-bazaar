        ...
        <main className="authPage">
          <div className="authWrap">
            <div className="authBrand">
              ...
              <h1>Sewa Bazaar</h1>
              <p>Fresh Organics, Delivered Daily</p>
            </div>

            <div className="authCard">
              <div className="authTabs">
                <button className={authMode === 'login' ? 'authTab active' : 'authTab'} onClick={() => { setAuthMode('login'); setError('') }}>Sign In</button>
                <button className={authMode === 'register' ? 'authTab active' : 'authTab'} onClick={() => { setAuthMode('register'); setError('') }}>Register</button>
              </div>

              {error && <div className="authError"><span>⚠</span> {error}</div>}
              ...
              {authMode === 'login' ? (
                <form onSubmit={handleLogin} className="authForm">
                  ...
                  <p className="authSwitch">
                    New to Sewa Bazaar?{' '}
                    <button type="button" onClick={() => { setAuthMode('register'); setError('') }}>Create account</button>
                  </p>
                </form>
              ) : (
          ...
          <style jsx>{`
            .authPage { background: #f1f3f6; min-height: calc(100vh - 120px); display: grid; place-items: center; padding: 40px 16px; }
            .authWrap { width: 100%; max-width: 480px; }
            .authBrand { text-align: center; margin-bottom: 28px; }
            .authBrandIcon { width: 60px; height: 60px; background: #619233; border-radius: 50%; display: grid; place-items: center; margin: 0 auto 14px; }
            .authBrand h1 { margin: 0 0 4px; font-size: 22px; font-weight: 700; color: #212121; font-family: 'Playfair Display', serif; }
            .authBrand p { margin: 0; font-size: 13px; color: #777; }
            .authCard { background: #fff; border-radius: 10px; box-shadow: 0 4px 24px rgba(0,0,0,0.08); overflow: hidden; }
            .authTabs { display: grid; grid-template-columns: 1fr 1fr; border-bottom: 1px solid #f0f0f0; }
            .authTab { background: none; border: none; padding: 16px; font-size: 15px; font-weight: 600; cursor: pointer; color: #888; transition: all 0.2s; }
            .authTab.active { color: #619233; border-bottom: 2px solid #619233; margin-bottom: -1px; }
            .authError { margin: 16px 24px 0; padding: 10px 14px; background: #fff2f2; border: 1px solid #fcc; border-radius: 6px; color: #c0392b; font-size: 13px; display: flex; align-items: center; gap: 8px; }
            .authForm { padding: 24px 24px 28px; display: flex; flex-direction: column; gap: 16px; }
            .formRow2 { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
            .formFld { display: flex; flex-direction: column; gap: 5px; }
            .formFld label { font-size: 12px; font-weight: 700; color: #555; text-transform: uppercase; letter-spacing: 0.4px; }
            .formFld input { padding: 10px 13px; border: 1.5px solid #ddd; border-radius: 7px; font-size: 14px; color: #212121; transition: border-color 0.2s; outline: none; background: #fafafa; }
            .formFld input:focus { border-color: #619233; background: #fff; }
            .authPrimaryBtn { background: #619233; color: #fff; border: none; padding: 13px; border-radius: 7px; font-size: 15px; font-weight: 600; cursor: pointer; transition: background 0.2s; margin-top: 4px; }
            .authPrimaryBtn:hover:not(:disabled) { background: #4f7a29; }
            .authPrimaryBtn:disabled { opacity: 0.6; cursor: not-allowed; }
            .authSwitch { margin: 4px 0 0; text-align: center; font-size: 13px; color: #666; }
            .authSwitch button { background: none; border: none; cursor: pointer; color: #619233; font-weight: 600; font-size: 13px; padding: 0; }
