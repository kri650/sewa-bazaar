...
export default function ProductCard({
  name, price, size, image, badge, description, note,
  showQty = false, qty = 1, onQtyChange, onAdd,
}) {
  const numeric = parseRupees(price)
  const displayPrice = showQty
    ? formatRupees(numeric * qty)
    : (typeof price === 'string' && price.trim().startsWith('Rs') ? price : formatRupees(numeric))
